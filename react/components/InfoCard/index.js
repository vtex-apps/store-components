...
const getImageUrl = (isMobile, imageUrl, mobileImageUrl) => !!mobileImageUrl && isMobile ? mobileImageUrl : imageUrl
...
const InfoCard = ({
  ...
  const containerStyle = isFullModeStyle ? { backgroundImage: `url(${finalImageUrl})`, backgroundSize: 'cover' } : {}
  ...
  return (
    <div className={containerClasses} style={containerStyle}>
      <div className={textContainerClasses}>
        ...
        {subhead && <p className={`${styles.infoCardSubhead} t-body mt6 c-on-base ${alignToken}`}>{subhead}</p>}
        <CallToAction mode={callToActionMode} text={callToActionText} url={callToActionUrl} />
      </div>
      {!isFullModeStyle && <img className="w-50-ns" src={finalImageUrl} style={{ objectFit: 'cover' }}/>}
    </div>
  )
}
