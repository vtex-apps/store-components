...
| `inputType` | `String` | Whether the search input should be type `search` or `text`. Prefer using `search` as it fixes some iOS issues. `text` is kept for backwards compatibility. | `text` |
