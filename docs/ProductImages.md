...
| `ModalZoom`               | `block`                                  | Opens a modal for product image zooming. This prop value must match the name of the block responsible for triggering the modal containing the product image for zooming (e.g., `modal-layout` from [Modal layout](https://developers.vtex.com/apps/guides/vtex.modal-layout) app). Please note that the `ModalZoom` prop will only work if the `zoomMode` prop is set as `open-modal`. To learn more, please see the [Advanced Configuration section](#Advanced-Configuration).                 | `undefined`      |
