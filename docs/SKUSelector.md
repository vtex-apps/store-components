...
| `possibleToClickOnImpossibleCombination`  | `array` | In case a product variation leading to a product combination is impossible, but you want to be able to click on this option anyway. The name of the specification that you want to have this behavior must be placed inside the array. It is also possible to put the "all" option inside the array so that this behavior is applied to all specifications | `undefined` |
